...
def train(
    ...
    @jax.jit
    def run_eval(params, state, normalizer_params, key) -> Tuple[envs.State, PRNGKey]:
    ...
    def do_one_step(carry, step_index):
        ...
        return (nstate, params, normalizer_params, key), (nstate.reward, state.obs)
    ...
    def _minimize(training_state: TrainingState, state: envs.State):
        ...
        grad, normalizer_params = loss_grad(
            training_state.policy_params,
            training_state.normalizer_params,
            state,
            key_grad,
    ...
    eval_sps = 0
    summary = {
        "params_norm": optax.global_norm(jax.tree_map(lambda x: x[0], policy_params))
    }
