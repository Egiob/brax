...
def train(
    ...
    def run_es_eval(state, params, key, normalizer_params):
        ...
        (state, _, key, cumulative_reward, _, _), (obs, obs_weights) = jax.lax.scan(
            do_one_step,
            (state, params, key, cumulative_reward, active_episode, normalizer_params),
            (),
            length=episode_length // action_repeat,
        )
    ...
    def add_noise(params, key):
        ...
        params_with_noise = jax.tree_map(
            lambda g, n: g + n * perturbation_std, params, noise
        )
        anit_params_with_noise = jax.tree_map(
            lambda g, n: g - n * perturbation_std, params, noise
        )
    ...
    def es_one_epoch(state: envs.State, training_state: TrainingState):
        ...
        params_with_noise, params_with_anti_noise, noise = add_noise(params, key_petr)
        ...
        eval_scores, obs, obs_weights, state = prun_es_eval(
            state, pparams, key_es_eval, training_state.normalizer_params
        )
        ...
        if fitness_shaping == 1:
            ...
            weights = jnp.maximum(
                0, jnp.log(weights.shape[0] / 2.0 + 1) - jnp.log(weights)
            )
        ...
        delta = jax.tree_map(
            ...
            * jnp.sum(
                y
                * jnp.reshape(weights, ([weights.shape[0]] + [1] * (len(y.shape) - 1))),
                axis=0,
            ),
            noise,
        )
        ...
        # perturbations.
        delta = jax.tree_map(
            lambda d, th: d - l2coeff * th, delta, training_state.policy_params
        )
    ...
    while True:
        ...
        t = time.time()
        num_process_env_steps = int(training_state.normalizer_params[0]) * action_repeat
