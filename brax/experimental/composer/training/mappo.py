...
def compute_ppo_loss(
    ...
    extra_loss_fns: Optional[Dict[str, Callable[[ppo.StepData], jnp.ndarray]]] = None,
...
def train(
    ...
    make_models_fn: Optional[
        Callable[[int, int], Tuple[networks.FeedForwardModel]]
    ...
    first_state = jax.tree_map(lambda *args: jnp.stack(args), *tmp_env_states)
    ...
    policy_params = jax.tree_map(lambda x: x[0], get_params(training_state, "policy"))
    extra_params = jax.tree_map(lambda x: x[0], get_params(training_state, "extra"))
